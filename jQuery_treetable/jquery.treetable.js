...
var ThisIframeId = "";
		...
		$.extend({
			treetable: new function() {
				...
				this.construct = function(settings){
					...
					var expandable = config.expandable;
					...
					$this.find("tr").each(function(){
						...
						tr_arr.push({'id':id,'parent':parent,'level':0,'node':'leaf','expanded':expandable,'obj':$(this)});
					});
					...
					*/
							...
							if(o.parent==""){//根行直接压入tr_sort
								...
								for(var j=0;j<tr_sort.length;j++){
									if(tr_sort[j].id==o.parent){
										...
										var temp = j;
										if(config.order_col == 1){
											temp = MaxTreeIndex(tr_sort, j);
										}
										
										tr_sort.splice(temp+1,0,o);//数组插入
										tr_arr[i]=null;
										len=len-1;
										//init_root_flag = false;
										break;
									}
								}
								/*if(init_root_flag){
					...
					for(var j=tr_sort.length-1;j>-1;j--){//prepend插入tbody内需使用反序
						...
						var img = $("<img class='jiajian' style='' src='"+config.open_img+"' />");
						img.click(fn_click);
						...
						if(o.node!='leaf')
							{
						//tr.prepend("<i style='padding-left:4px'></i>");8
						tr.prepend(img);
						}else{
							tr.prepend("<i style='padding-left:20px'></i>");
						}
						var s = new Array((o.level+1)*5).join("<i style='padding-left:4px;'></i>");//生成缩进空格
						tr.prepend(s);
					...
					for(var j=0;j<tr_sort.length;j++){
						...
						if(!o.expanded&&o.node!='leaf'){//通过图标判断是展开还是收起
							img.src=config.open_img;
							o.expanded=true;
						}else if(o.node!='leaf'){
							img.src=config.close_img;
	...
	function MaxTreeIndex(tr_sort, j){
		var temp = j;
		for(var k=j+1;k<tr_sort.length;k++){
			if(tr_sort[k].parent==tr_sort[j].id){
				temp = k;
			}
		}
		if(temp > j){
			temp = MaxTreeIndex(tr_sort, temp);
		}
